...
# 启用中文字体支持
theme_font <- "STSongti-SC-Black"
font_add(family = "STSongti-SC-Black", regular = "data/STSongti-SC-Black.ttf")
```
...
create_boxplot <- function(data_raw, y_limit = NULL) {
  ggplot(data_raw, aes(x = "年薪", y = 年薪)) +
    ...
    theme_minimal() +
    theme(
      text = element_text(family = theme_font), # 默认中文字体
      axis.text = element_text(size = 10, color = "#666666", family = theme_font), # 坐标轴刻度
      axis.title = element_text(size = 12, face = "bold", color = job_palette["A"], family = theme_font), # 坐标轴标题
      axis.text.x = element_blank(), # 隐藏 x 轴刻度
      axis.title.x = element_blank(), # 隐藏 x 轴标题
      axis.text.y = element_text(size = 10, family = theme_font), # y 轴刻度中文显示
      plot.title = element_text(size = 16, face = "bold", color = job_palette["A"], family = theme_font), # 图标题
      plot.subtitle = element_text(size = 12, color = job_palette["B"], family = theme_font) # 副标题
    ) +
    scale_y_continuous(
      breaks = scales::pretty_breaks(n = 6), # 主坐标轴等间隔视觉距离
      labels = scales::number_format(accuracy = 0.1), # 对数坐标标签
      sec.axis = sec_axis(
        trans = ~ exp(.), # 次坐标轴转换回实际年薪
        name = "年薪（万元）",
        breaks = exp(scales::pretty_breaks(n = 6)(range(data_raw$log_salary))),
...
create_histogram <- function(data_raw, x_limit = NULL) {
  ggplot(data_raw, aes(x = 年薪)) +
    ...
    theme(
      ...
      axis.text = element_text(size = 10, color = "#666666", family = theme_font), # 坐标轴刻度
      axis.title = element_text(size = 12, face = "bold", color = job_palette["A"], family = theme_font), # 坐标轴标题
      axis.text.x = element_text(size = 10, family = theme_font), # x 轴刻度中文显示
      axis.text.y = element_text(size = 10, family = theme_font), # y 轴刻度中文显示
      plot.title = element_text(size = 16, face = "bold", color = job_palette["A"], family = theme_font), # 图标题
      plot.subtitle = element_text(size = 12, color = job_palette["B"], family = theme_font) # 副标题
    ) +
...
从五大地区职位数量分布图中可以看出：
1. 东部地区占据主导地位：
   - 东部地区的职位数量显著领先其他区域，显示了东部经济较为发达，吸引了更多企业和职位。
2. 中部地区次之：
   - 中部地区职位数量排名第二，表明中部地区在经济发展中逐渐崭露头角，对就业市场的吸引力有所增加。
3. 南部地区紧随其后：
   - 南部地区职位数量排名第三，其经济发展较快，尤其是在大湾区的带动下，创造了大量就业机会。
4. 西部地区职位较少：
   - 西部地区的职位数量较少，反映了西部地区相较于东部、中部的经济发展仍有一定差距。
5. 东北地区职位最少：
   - 东北地区的职位数量最少，可能与近年来东北地区经济发展放缓有关，工作岗位增长缓慢。
6. 区域经济差异明显：
   - 图表反映了不同区域经济发展水平的不均衡性，从而导致职位数量分布的差异。
7. 就业热点集中于东部和南部：
   - 企业对东部和南部地区的偏好凸显了这些区域的经济活跃度以及较高的人才吸引力。
...
p <- ggplot(
  ...
  theme(
    ...
    plot.background = element_blank(), # 移除整体背景
    panel.grid.major = element_line(color = "#E6E6E6"), # 主网格线颜色
...
从不同地区薪资分布图中可以看出：
1. 东部地区薪资水平最高  
   - 东部地区的薪资中位数和上四分位数显著高于其他区域，反映了东部经济的高度发达以及对高薪职位的巨大需求。  
   - 薪资分布范围最广，尾部较长，表明东部地区存在大量高薪岗位，例如技术或管理类职位。  
   - 箱线图显示，东部地区有多个高薪离群点，可能对应于顶尖岗位或高技术含量职位。

2. 南部地区次高  
   - 南部地区薪资水平紧随东部地区之后，薪资中位数和分布范围表现出较强竞争力。  
   - 小提琴图显示南部地区的尾部略短于东部，说明高薪职位比例略低，但薪资水平仍处于较高位置。

3. 中部地区薪资分布中等  
   - 中部地区薪资分布位于全国中游，薪资中位数低于东部和南部地区，但明显高于西部和东北地区。  
   - 小提琴图显示，薪资分布较均匀且尾部较短，说明高薪职位数量有限。

4. 西部地区薪资偏低  
   - 西部地区薪资中位数和分布范围显著低于东部和南部地区，反映了西部经济发展与东部的差距。  
   - 薪资分布范围较窄，尾部较短，离群点较少，表明西部地区高薪职位稀缺。

5. 东北地区薪资水平最低  
   - 东北地区的薪资中位数和分布范围均为最低，显示出东北地区在经济发展和就业市场上的劣势。  
   - 箱线图中分布范围较窄，尾部几乎不存在，表明高薪职位比例非常低。

6. 区域经济差异显著  
   - 东部和南部地区的薪资分布显著高于其他地区，表明经济发达地区更容易吸引高薪职位。  
   - 西部和东北地区的薪资水平偏低，显示了区域经济发展不平衡对薪资分布的直接影响。

### x2=职位

```{r}
# 描述性统计
job_summary <- data_raw %>%
  group_by(职位) %>%
  summarise(
    职位数量 = n(),
    平均年薪 = mean(年薪, na.rm = TRUE),
...
```{r}
# 描述性统计
work_experience_summary <- data_raw %>%
  group_by(福利) %>%
  summarise(
    职位数量 = n(),
    平均年薪 = mean(年薪, na.rm = TRUE),
...
# 绘制小提琴图
ggplot(data_raw, aes(x = 福利状态, y = log_salary, fill = 福利状态)) +
...
```{r}
# 绘制"已有工作经验"的小提琴图 + 箱线图
p_experience_salary <- ggplot(data_raw, aes(x = factor(已有工作经验, levels = c("经验不限", "1年以内", "1-3年", "3-5年", "5年以上")), y = log(年薪))) +
...
# 确保学历按照固定顺序排列
education_levels <- c("学历不限", "高中及以下", "大专", "本科", "研究生")
data_raw$学历 <- factor(data_raw$学历, levels = education_levels)

# 绘制学历与对数年薪的联合图（小提琴图 + 箱线图）
p_education_salary <- ggplot(data_raw, aes(x = 学历, y = log_salary, fill = 学历)) +
...
```{r}
# 小提琴图 + 箱线图（公司行业）
p_industry_combined <- ggplot(data_raw, aes(x = reorder(公司行业, -log_salary, FUN = median), y = log_salary)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
  labs(
    title = "不同公司行业的薪资分布",
...
```{r}
# 小提琴图 + 箱线图（融资阶段）
p_finance_combined <- ggplot(data_raw, aes(x = factor(融资阶段, levels = c("无需融资", "未融资", "天使轮", "A轮", "B轮", "C轮", "D轮及以上", "已上市", "未知")), y = log(年薪))) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
...
```{r}
# 小提琴图 + 箱线图（公司规模）
p_scale_combined <- ggplot(data_raw, aes(x = factor(公司规模, levels = c("100以下", "100-499", "500以上")), y = log(年薪))) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
  labs(
    title = "不同公司规模的薪资分布",
...
```{r}
# 小提琴图 + 箱线图（职业类别）
p_category_combined <- ggplot(data_raw, aes(x = reorder(职业类别, -log_salary, FUN = median), y = log_salary)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
  labs(
    title = "不同职业类别的薪资分布",
