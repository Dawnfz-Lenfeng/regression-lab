...
```{r}
# 小提琴图 + 箱线图（公司行业）
p_industry_combined <- ggplot(data_raw, aes(x = reorder(公司行业, -log_年薪, FUN = median), y = log_年薪)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") +  # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) +  # 箱线图
  labs(
    title = "不同公司行业的薪资分布",
...
```{r}
# 小提琴图 + 箱线图（职业类别）
p_category_combined <- ggplot(data_raw, aes(x = reorder(职业类别, -log_年薪, FUN = median), y = log_年薪)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") +  # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) +  # 箱线图
  labs(
    title = "不同职业类别的薪资分布",
