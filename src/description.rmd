...
create_boxplot <- function(data_raw, y_limit = NULL) {
  ggplot(data_raw, aes(x = "年薪", y = 年薪)) +
    ...
    theme(
      ...
      axis.text = element_text(
        size = 10, color = "#666666", family = theme_font
      ), # 坐标轴刻度
...
p <- ggplot(
  ...
  scale_y_continuous(
    breaks = seq(floor(min(data_raw$log_salary, na.rm = TRUE)),
      ceiling(max(data_raw$log_salary, na.rm = TRUE)),
      by = 0.5
    ),
...
```{r}
# 小提琴图 + 箱线图（公司行业）
p_industry_combined <- ggplot(data_log, aes(x = reorder(公司行业, -log_salary, FUN = median), y = log_salary)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
  labs(
    title = "不同公司行业的薪资分布",
...
```{r}
# 小提琴图 + 箱线图（职业类别）
p_category_combined <- ggplot(data_log, aes(x = reorder(职业类别, -log_salary, FUN = median), y = log_salary)) +
  geom_violin(fill = "#85C1E9", alpha = 0.7, width = 0.8, scale = "width") + # 小提琴图
  geom_boxplot(width = 0.2, fill = "white", alpha = 0.6, outlier.shape = NA) + # 箱线图
  labs(
    title = "不同职业类别的薪资分布",
